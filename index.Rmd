...
#Graficacion
highchart() %>% hc_title(text="Modulacion de las ecuaciones", useHTML = T) %>% hc_add_theme(hc_theme_db()) %>%hc_xAxis(title=list(text="Tiempo")) %>% hc_yAxis(title = list(text="Valores", useHTML=T), min=-1.3, max = 1.3, allowDecimal = T) %>% hc_chart(zoomType="xy") %>% hc_add_series(cbind(x,am), name="AM") %>% hc_add_series(cbind(x,ft), type= "line", dashStyle = "Dash", name="ecuacion 1", color = "#bd4f6c") %>% hc_add_series(cbind(x,f2), type= "line", dashStyle = "Dash", name="Ecuacion 2", color = "#f0cf65") %>% hc_add_series(cbind(x,f3), type= "line", dashStyle = "Dash", name="Ecuacion 3", color = "#93b5c6") 
```



#AM demodulacion de las ecuaciones
...
demAM2 <- am2*crr
demAM3 <- am3*crr
highchart() %>% hc_add_series(cbind(t,demAM), name="AM-DEM") %>% hc_add_series(cbind(t, ft), dashStyle = "Dash", name="Ec 1", color = "#bd4f6c") %>% hc_add_series(cbind(t, f2), dashStyle = "Dash", name="Ec 2", color = "#f0cf65") %>% hc_add_series(cbind(t, f3), dashStyle = "Dash", name="Ec 3", color = "#93b5c6") %>%  hc_title(text="Modulación AM de una señal AM", useHTML=T) %>% hc_add_theme(hc_theme_db()) %>% hc_xAxis(title=list(text="Tiempo")) %>% hc_yAxis(title = list(text="Valores", useHTML=T), min=-1.8, max = 1.8, allowDecimal = T) %>% hc_chart(zoomType="xy")
```
